...
const PDFName = 'Glynwood Center Adams Label - Template.pdf';
const CSVName = 'adams-cuts.csv';
let fileCount = 0;
...
async function main() {
  ...
  for (let i = 1; i < cuts.length; i++ ) {
    const cut = cuts[i];
    const slug = slugify(cut[0] + ' ' + cut[1]);

    setDynamicFields(form, cut);

    await savePDF(pdfDoc, `./adams-outputs/${PDFName.replace('Template', slug)}`);
  }
